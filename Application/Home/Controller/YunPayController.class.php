...
class YunPayController extends HomeController {
    ...
    private $pc_load_url;
    private $jubaoyun;
    private $wap_load_url;

    public function __construct($conf) {

        $this->pc_load_url = C('pc_load_url');
        $this->jubaoyun = C('jubaoyun');
        $this->wap_load_url = C('wap_load_url');
    }
    ...
    public function index($pay_token = '') {
        $pay = S($pay_token);
        $payid = $pay['WIDout_trade_no']; //订单号
        $amount = $pay['WIDtotal_fee']; //金额
        $payerName = $pay['uid']; //支付用户
        $remark = $pay['WIDsubject']; //商品备注
        $returnURL = $this->pc_load_url . "/cgyyg1.0/index.php/Home/YunPay/result";   // 可在商户后台设置【同步页面】
        $callBackURL = $this->pc_load_url . "/cgyyg1.0/index.php/Home/YunPay/result"; // 可在商户后台设置【异步通知】
        $os = $pay["os"]; //移动端还是pc端判断
        $pay_url = ($os == 1) ? $this->jubaoyun["payurl_wap"] : $this->jubaoyun["payurl_pc"]; //提交支付页面
        $payMethod = ($os == 1) ? "" : "ALL"; //付款方式
        ...
        $jubaopay->setEncrypt("partnerid", $this->jubaoyun["partnerid"]);
    ...
    public function result() {
        ...
        if ($result == 1) {
            A("Home/Log")->setlog("校验签名成功", __LINE__, __METHOD__);
            $payid = $jubaopay->getEncrypt("payid"); //订单号
            $mobile = $jubaopay->getEncrypt("mobile"); //预留手机号
            ...
            $payMethodType = $jubaopay->getEncrypt("payMethodType"); //支付类型  网银【银卡、支付宝、微信】
            $payMethod = $jubaopay->getEncrypt("payMethod"); //实际支付方式 wxpay 
            $payMethod = $this->payMethod($payMethod);
            $pay_token = S($payid);
            $payinfo = S($pay_token);
            $money = $payinfo["WIDtotal_fee"];
            $money = (preg_match("/^[1-9]\d*$/", $money)) ? $money . ".00" : $money;
            $pay_num = $payinfo['WIDout_trade_no']; //订单号
            $partner_id = $this->jubaoyun["partnerid"];
            A("Home/Log")->setlog("回调参数判断", __LINE__, __METHOD__);
            if (($partnerid == $partner_id) && ($amount == $money) && ($state == 2) && ($pay_num == $payid)) {//判断第三方回调是否正确
                A("Home/Log")->setlog("回调参数判断成功", __LINE__, __METHOD__);
                if ($payinfo["pay_type"] == 1) {
                    $this->recharge($pay_token, $orderNo, $payMethod);
                } else {
                    $this->repayfor($pay_token, $orderNo, $payMethod);
                }
            } else {
                echo json_encode(array('code' => 500, 'info' => '支付失败'));
                A("Home/Log")->setlog("回调参数判断失败", __LINE__, __METHOD__);
            }
        }
    }
    ...
    public function payMethod($payMethod = 0) {
        switch ($payMethod) {
            case wxpay://微信
                $payMethod = 1;
                break;
    ...
    public function recharge($pay_token = '', $orderNo = '', $payMethod = 0) {
        $payinfo = S($pay_token);
        $os = $payinfo["os"]; //判断pc还是手机
        $uid = floatval($payinfo["uid"]); //缓存的uid
        $money = $payinfo["WIDtotal_fee"]; //缓存的金额
        $data = array(
            "order_number" => $payinfo["WIDout_trade_no"], //商户订单号
            "trade_no" => $orderNo, //云支付交易号
            "price" => $payinfo["WIDtotal_fee"], //价格
            "trade_time" => date("Y-m-d H:i:s", time()), //交易时间
            "trade_type" => 1, //交易类型,1充值，2支付
            "uid" => $uid, //用户id
            "create_time" => time(), //创建时间
            "charge_type" => 1, //充值类型
            "charge_number" => $payinfo["WIDout_trade_no"], //充值订单号
            "money" => $money, //金额 
        ...
        if (!$insert) {
            ...
            if ($rechargeid > 0 && $insert_id > 0) {//充值成功
                ...
                $total_account = floatval($money) + floatval($account);
                ...
                echo "success";
                A("Home/Log")->setlog("支付异步success", __LINE__, __METHOD__);
            } else {//充值失败
                $model->rollback();
        ...
        if ($os == 1) {
            header("Location: " . $this->wap_load_url . "/35-personal.html?c");
        } else {
            header("Location: " . $this->pc_load_url . "/personal-index.html?c");
        }
    }
    ...
    public function repayfor($pay_token = '', $orderNo = '') {
        ...
        if ($insert) {
            ...
            A("Home/Log")->setlog("支付入库recharge", $recharge_arr, __METHOD__);
                    ...
                    foreach ($array['list'] as $value) {
                        ...
                        if ($value['attend_count'] <= $remain_count) { //参加次数小于或等于剩余次数
                            ...
                            for ($i = 0; $i < $value['attend_count']; $i++) {  //随机获得幸运码和删除已分配的幸运码
                                ...
                                //A("Home/Log")->setlog("删除已分配的幸运码", __LINE__, __METHOD__);
                            }
        ...
        if ($os == 1) {
            header("Location: " . $this->wap_load_url . "/19-result.html?p");
        } else {
            header("Location: " . $this->pc_load_url . "/payfor3.html?p");
        }
    }
    ...
    public function charge() {
        ...
        $this->index($charge_token);
        A("Home/Log")->setlog("充值第一步[跳转第三方支付页面前]", $data, __METHOD__);
    }
    ...
    public function payfor() {
        ...
        $this->index($pay_token);
        A("Home/Log")->setlog("购买第一步[跳转第三方支付页面前]", $data, __METHOD__);
    }
