...
class LotteryController extends HomeController {
    ...
    public function validateAttend() {
        ...
        if (isset($array)) {
            ...
            foreach ($array['list'] as $value) {
                $arr = D('lotteryAttend')->validateAttendProduct($value, $uid); //验证购买数量是否有效
                if ($arr['code'] == 503) {
            ...
            if ($flag == -1) {
                ...
                $arr = D('lotteryAttend')->valdateBalance($uid, $total, $money, $payMoney, $balance, $payType);
                $arr['list'] = array();
        ...
        if ($array != null) {
            ...
            if ($bal_arr['code'] == 200) {
                if ($payMoney == 0 && $balance > 0) { //选择余额支付，余额充足，用余额支付全额
                    $state = D('lotteryAttend')->plusBalance($uid, $money); //在用户余额上扣除金额
                    if ($state == -1) {
    ...
    function attendLottery() {
        ...
        //热门推荐
        $hotProduct = D('lotteryProduct')->getHotList(4);
        if (S($uid)) {
            $arr = array(
                'code' => 200,
                'info' => '成功',
    ...
    public function executelotteryResult() {
        D('lotteryAttend')->lotteryResultNoCqssc();
    }
    ...
    public function LotteryNew() {
        ...
        if (!$ssc_data) {
            \Think\Log::write('时时彩出问题了啊，快检查', 'WARN');
            exit;
        }
        ...
        $hour_time = $ssc_data['ssc_time']; //最新一期时时彩开奖时间
        $add_time = $ssc_data['add_time'];
        $field = 'lottery_id,pid,need_count,total_time';
        //获取时时彩开奖时间之前所有即将揭晓商品的记录
        $lottery = M('lottery_product')->field($field)->where('expectTime > 0 and lottery_code=0 and expectTime <=' . $add_time)->select();
        if (!$lottery) {
            echo '没有要开的奖';
            exit;
        }
        ...
        for ($i = 0; $i < $len; $i++) { //一条一条开奖
            ...
            $data = array();
            $data['hour_lottery'] = $ssc_data['number']; //时时彩号码
