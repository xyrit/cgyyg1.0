...
class UcenterFocusController extends HomeController {
    ...
    public function focusProduct() {
        ...
        $flag = D("documentProduct")->focusProduct($uid, $pid, $lotteryId);
        if ($flag == -1) {
    ...
    public function focusList() {
        ...
        $this->is_login($user_token); //判断是否登录
        $uid = floatval(S($user_token));
        $total = D('documentProduct')->focusCount($uid);
        $list = D('documentProduct')->focusList($uid, $pageIndex, $pageSize);
    ...
    public function readProduct() {
        ...
        $this->is_login($user_token); //判断是否登录
        $uid = floatval(S($user_token));
        $flag = D("documentProduct")->readProduct($uid, $pid, $lotteryId);
        if ($flag == -1) {
    ...
    public function readList() {
        ...
        $this->is_login($user_token); //判断是否登录
        $uid = floatval(S($user_token));
        $total = D('documentProduct')->readCount($uid);
        $list = D('documentProduct')->readList($uid, $pageIndex, $pageSize);
        $pageCount = floor($total / $pageSize);
        if ($total % $pageSize > 0) {
            $pageCount = $pageCount + 1;
        }
