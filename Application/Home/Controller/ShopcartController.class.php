...
class ShopcartController extends HomeController {
    ...
    public function changeNum() {
        ...
        if (!empty($_POST)) {
            ...
            foreach ($arrs as $k => $v) {
                ...
                $data['attend_count'] = $attend_count;
                $count = D("shopcart")->remainCount($id); //获取该购物车id的购买人次限制信息
                ...
                D("shopcart")->cartSave($id, $uid, $data); //保存更改的购物车信息
                $payfor = D("shopcart")->getPaylist($id, $uid); //返回修改后的商品信息
    ...
    public function index() {
        ...
        if ($uid > '0') {//已登录
            ...
            $data['HotList'] = D("LotteryProduct")->getHotList(4); //热门推荐         
    ...
    public function hotList() {
        $user_token = I("post.user_token");
        $uid = $this->is_login($user_token);
        $hotList = D("LotteryProduct")->getHotList(4); //热门推荐     
        echo json_encode(array('code' => 200, 'info' => '成功', 'host' => C('HOST'), 'hotList' => $hotList));
    }
