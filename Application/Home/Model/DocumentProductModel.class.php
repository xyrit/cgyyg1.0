...
class DocumentProductModel extends Model {
     ...
     * 参与明细(进行中、即将揭晓、已揭晓)
     */
    public function attendNowList($uid, $pageIndex, $pageSize, $timeStart, $timeEnd, $state) {
        $prefix = C('DB_PREFIX');
        ...
        if ($state == '0') {//进行中商品
            ...
            $WHERE = " WHERE ((la.uid=" . $uid . ")" . $BETWEEN . " and (lp.last_attend_time>0) and (lp.lottery_code=0) and (lp.uid<1)) ";
            $condition = 'create_time asc';
        } else if ($state == '2') {//已揭晓商品
            $WHERE = "WHERE ((la.uid=" . $uid . ")" . $BETWEEN . " and (lp.lottery_time>0) and (lp.uid>0)) ";
            $condition = 'lottery_time desc';
        }
        $sql = "select * from (select max(la.id)as id , la.lottery_id, la.uid, " .
                " sum(la.attend_count)as attend_count,d.title ,la.lucky_code,lp.need_count,lp.attend_count as attend_all, p.path, " .
                " max(la.create_time)as create_time , " .
                " lp.lottery_code,lp.lottery_time,lp.pid,m.nickname,lp.uid as lp_uid" .
                " from " . $prefix . "member as m right join " . $prefix . "lottery_attend as la on m.uid=la.uid" .
                " left join " . $prefix . "lottery_product as lp on la.lottery_id=lp.lottery_id left join os_document as d on lp.pid = d.id" .
                "  left join " . $prefix . "picture as p on d.cover_id = p.id " .
                $WHERE . " group by la.lottery_id  asc   ) a order by " . $condition . " limit $start,$end;  ";
        //echo $sql;
        $list = $this->query($sql);
        if ($state == '2') {
            foreach ($list as $k => $v) {
                $list[$k]["lp_attend_count"] = $list[$k]["attend_count"];
                if ($list[$k]["lp_uid"] != $uid) {//如果中奖者不是当前用户
                    $lp_uid = $list[$k]["lp_uid"];
                    $lottery_id = $list[$k]["lottery_id"];
                    $sql2 = "select la.lottery_id,la.attend_count,m.nickname from " . $prefix . "member as m right join " .
                            $prefix . "lottery_attend as la on m.uid=la.uid  WHERE (la.lottery_id=" . $lottery_id . " and la.uid = " . $lp_uid . ") group by la.id desc ";
                    $prize = $this->query($sql2);
                    $list[$k]["nickname"] = $prize[0]["nickname"];
                    $list[$k]["lp_attend_count"] = $prize[0]["attend_count"];
                }
            }
        }
        $result = array(); //存储最终结果
        $lucky_arr = array(); //存储lucky_code
        $time_arr = array(); //存储时间
        foreach ($list as $v) {
            $result[$v['lottery_id']] = $v; //将结果集赋给新数组，以结果集的lottery_id作为新数组的键
            $time_arr = explode("|", $result[$v['lottery_id']]["create_time"]); //切割成数组
            $result[$v['lottery_id']]["new"]["create_time"] = $time_arr; //将create_time放入临时数组new
            $lucky_arr = explode("|", $result[$v['lottery_id']]["lucky_code"]); //切割成数组
            $result[$v['lottery_id']]["new"]["lucky_code"] = $lucky_arr; //将lucky_code放入临时数组new
            $result[$v['lottery_id']]["time_code"] = $this->turnArrays($result[$v['lottery_id']]["new"]); //调用数组合并函数
        }
        $result = array_values($result);
        foreach ($result as $k => $v) {
            unset($result[$k]["new"]); //删除临时数组new
            unset($result[$k]["lucky_code"]); //删除多余的lucky_code
            unset($result[$k]["create_time"]); //删除多余的create_time
            $result[$k]["remain_count"] = $result[$k]["need_count"] - $result[$k]["attend_all"]; //计算剩余人次
            rsort($result[$k]["time_code"]);
        }
        return $result;
    }
