...
class AttendController extends HomeController {
    ...
    public function attendDetail() {
        ...
        $transForm = A("Home/ucenter")->transForm($soso, $start_Time, $end_Time); //转换时间戳
        $timeStart = $transForm[0];
        $timeEnd = $transForm[1]; //dump($transForm);
        if ($uid > 0) {
            $pageSize = I('post.pageSize'); //每页数量
            $pageIndex = I('post.pageIndex'); //第几页，从0开始
            $total = D('Home/DocumentProduct')->getAttendCount($uid, $pageIndex, $pageSize, $timeStart, $timeEnd, $state); //获取总记录echo $total;
            $pageCount = UtilApi::getPage($pageSize, $total); //获取分页
            $list = D('Home/DocumentProduct')->attendNowList($uid, $pageIndex, $pageSize, $timeStart, $timeEnd, $state); //获取商品所有信息
            $arr = array(
                'code' => 200,
                'info' => '成功',
                'host' => C('HOST'),
    ...
    public function getAttendInfo() {
        ...
        if ($uid > 0 && $lottery_id > 0) {
            $result = D('Home/DocumentProduct')->getAttendInfo($uid, $lottery_id); //获取用户的参与时间、参与码集合
            $list = $result["list"];
            $sum = $result["sum"];
            $arr = array(
                'code' => 200,
