...
class AddressController extends HomeController {
    ...
    public function addressList() {
        if (IS_POST && !empty($_POST)) {
            ...
            $addressid = floatval(I("post.addressid"));
            $list = D("Home/Address")->getlist($uid, $addressid);
            $total = 3;
    ...
    public function addressAdd() {
        if (IS_POST && !empty($_POST)) {
            ...
            $data['verify'] = floatval(I('post.verify'));
            ...
            $length = strlen($data['verify']);
            if ((!preg_match("/^[1-9]\d*$/", $data['verify'])) || $length != '4') {
                echo json_encode(array('code' => 500, 'info' => '验证码必须是四位数的数字'));
                return;
            }
    ...
    public function getProvinces() {
        if (IS_POST && !empty($_POST)) {
            ...
            $state = I('post.state');
            $ProvincesList = D("Home/address")->getArea(0); //取出所有的省级
            if ($state == '1') {//省id对应的市
                $ProvincesList = D("Home/address")->getArea($pid);
    ...
    public function defaultSet($id = '', $uid = '') {
        ...
        $status_code = D("Home/address")->addressSet($id, $data); //设置默认地址
