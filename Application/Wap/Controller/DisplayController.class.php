...
class DisplayController extends HomeController {
    ...
    public function orderRecord($uid = '') {
        ...
        if ($uid > 0) {
            ...
            $total = D('Home/DocumentProduct')->orderRecordCount($uid, $pageIndex, $pageSize, $timeStart, $timeEnd, $state); //dump($total);
            $pageCount = UtilApi::getPage($pageSize, $total);
            $list = D('Home/DocumentProduct')->orderRecord($uid, $pageIndex, $pageSize, $timeStart, $timeEnd, $state);
            $picarr = array();
            foreach ($list as $k => $v) {
    ...
    public function orderInfo() {
        ...
        $order_info = D('Home/DocumentProduct')->orderInfo($id);
