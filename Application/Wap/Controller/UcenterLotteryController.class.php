...
class UcenterLotteryController extends HomeController {
    ...
    public function lotteryRecord() {
        ...
        $transForm = A("Home/ucenter")->transForm($soso, $start_Time, $end_Time);
        $timeStart = $transForm[0];
        $timeEnd = $transForm[1]; //dump($transForm);
        if ($uid > 0) {
            $total = D('Home/DocumentProduct')->lotteryRecordCount($uid, $pageSize, $pageIndex, $timeStart, $timeEnd, $state); //echo $total;
            $pageCount = UtilApi::getPage($pageSize, $total);
            $list = D('Home/DocumentProduct')->lotteryRecord($uid, $pageSize, $pageIndex, $timeStart, $timeEnd, $state);
            $arr = array('code' => 200,
                'info' => '成功',
                'host' => C('HOST'),
