...
use Addons\Verifycode\ValidateCode; //引入验证码类
...
class UserController extends HomeController {
    ...
    public function register_check($mobile = "") {
        if (!empty($_POST["mobile"])) {
            ...
            $state = $data["state"];
            $uid = D("Home/Member")->getUid($tel);
            if (!empty($othertoken)) {
                $uid = 0;
            }
            if ($uid > 0) {//查询用户是否已注册
                UtilApi::getInfo(503, '该手机号已注册');
                return;
            } else {
                $last_time = S($tel);
                ...
                if ($status_code == '200') {
                    ...
                    S(array('expire' => 24 * 3600)); //设置缓存有效期1h，以秒为单位
                    S($tel, time()); //存储手机号token
    ...
    public function verify() {
        if (!empty($_POST["mobile"]) && !empty($_POST["verify"])) {
            ...
            if (!empty($invite_code)) {//邀请码不为空检测
                ...
                if (!$code) {
                    UtilApi::getInfo(500, '邀请码不存在');
                    exit;
                }
            }
            $verify = intval(I("post.verify"));
            ...
            $res_data["cellphone"] = $data["mobile"];
            $res_data["verify"] = $verify;
            $res_data["type"] = 1; //短信类型
            if ($verify < 1) {
                UtilApi::getInfo(500, '短信验证码不正确');
                return;
            }

            $this->checkMobile($data["mobile"]); //检查手机号
            $verify_info = D("Home/Member")->getVerify($res_data); //得到验证码信息
            if (!empty($verify_info['creat_time']) && ((floatval($verify_info['creat_time']) + 6000) < time())) {//将获取的缓存时间转换成时间戳加上600秒后与当前时间比较，小于当前时间即为过期          
                echo json_encode(array('code' => 507, 'info' => "短信验证码已过期，请重新获取！"));
            } else {
                if (floatval($verify_info['verify']) != $verify) {
                    echo json_encode(array('code' => 500, 'info' => "短信验证码不正确"));     var_dump(55); 
                    exit;
                } else {
                    ...
                    M("verify")->where('cellphone=' . $verify_info['cellphone'])->save(array("status" => 1, "type" => -1));
                    exit;
                }
            }
        } else {
            echo json_encode(array('code' => 509, 'info' => "手机号或短信验证码不能为空"));
            exit;
        }
    }
    ...
    public function register($password = "", $mobile_token = "") {
        if (!empty($_POST["mobile_token"]) && !empty($_POST["password"]) && !empty($_POST["repassword"])) {
            ...
            if (!empty($data["password"]) && ($data["password"] != $data["repassword"])) {
                ...
                if ($uid > 0) {
                    //A("Commission")->CreateInvite_code($uid); //生成专属邀请码,二维码
                    A("Home/Activity")->activityScan_code();
                    if (!empty($invite_code)) {
                        // A("Commission")->bind_user($uid, $invite_code); //绑定用户
                    }
                    $this->login($data["mobile"], $data["password"], 0, 1, $load); //调用登陆,激活各应用同步登陆
     ...
     */
    public function login($mobile = "", $password = "", $type = '', $register = '', $load = 0, $other = 0) {// dump($_POST);
        $type = floatval(I("get.type"));
        if (empty($load)) {
            $load = floatval(I("get.load"));
        }
        switch ($type) {
            case 1://qq登录
            ...
            default:
                ...
                if (!empty($_POST["mobile"]) && !empty($_POST["password"])) {
                    ...
                    /* 登录用户 */
                    $member = D("Home/Member")->getUser($data); //查询用户信息
                    ...
                    if ($uid > 0) {//登录成功                  
                        ...
                        if ($other == '1') {
                            ...
                            $load_url = pc_load_url . "/login_guide.html?token=" . $other_token;
                            $memberinfo["load_url"] = $load_url;
                            echo json_encode(array('code' => 200, 'info' => '登陆成功', 'user' => $memberinfo));
                            exit;
                        } else {
    ...
    public function logout() {
        ...
        //$logout = A("Home/UcService")->uc_logout();
    ...
    public function check_res($mobile = '', $other_token = '') {
        ...
        $data["mobile"] = $mobile;
        $user_info = M("member")->field('mobile,password')->where($data)->find();
        if ($user_info) {
            ...
            $bind = M("member")->where($data)->save($datas);
            if ($bind > 0) {
    ...
    public function unbind() {
        $mobile = floatval(I("get.mobile"));
        $data["mobile"] = $mobile;
        $type = floatval(I("get.type")); //应用类型
        switch ($type) {
            case 1://qq登录
                $datas["qq_openid"] = '';
                break;
